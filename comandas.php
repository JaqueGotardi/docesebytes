...
// Criar Comanda
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['criar_comanda'])) {
    $observacoes = $_POST['observacoes'];

    $stmt = $conn->prepare("INSERT INTO comandas (observacoes, status) VALUES (?, 'Não iniciado')");
    $stmt->bind_param("s", $observacoes);
    $stmt->execute();
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['adicionar_item'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO itens_comandas (comanda_id, item_id) VALUES (?, ?)");
    $stmt->bind_param("ii", $comanda_id, $item_id);
    $stmt->execute();
    $stmt->close();
}
...
// Deletar Comanda
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['deletar_comanda'])) {
    $comanda_id = $_POST['comanda_id'];

    $stmt = $conn->prepare("DELETE FROM comandas WHERE id = ?");
...
<body>
    ...
    <form method="POST" class="form-container">
        <textarea id="observacoes" name="observacoes" rows="3" placeholder="Digite observações (opcional)..."></textarea>
        <button type="submit" name="criar_comanda">Criar Comanda</button>
    </form>
            ...
            <td>
                ...
                <form method="POST" style="margin-bottom: 10px;">
                    ...
                    <select name="item_id" required>
                        ...
                        <option value="<?= $item['id'] ?>"><?= $item['nome'] ?> (R$ <?= number_format($item['preco'], 2, ',', '.') ?>)</option>
                        <?php endforeach; ?>
                    </select>
                    <button type="submit" name="adicionar_item" style="background-color: #9e1fe0; color: white; border: none; padding: 10px 20px; font-size: 14px; border-radius: 5px; cursor: pointer;">Adicionar</button>
                </form>

                <!-- Formulário de Confirmação de Exclusão -->
                <form method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta comanda?');">
                    <input type="hidden" name="comanda_id" value="<?= $comanda['comanda_id'] ?>">
                    <button type="submit" name="deletar_comanda" style="background-color: #9e1fe0; color: white; border: none; padding: 10px 20px; font-size: 14px; border-radius: 5px; cursor: pointer;">Excluir</button>
                </form>
            </td>
        </tr>
        <?php endwhile; ?>
